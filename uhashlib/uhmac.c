...
static mp_obj_t hmac_HMAC_make_new_helper(const mp_obj_type_t *type, size_t n_args, const mp_obj_t *pos_args, mp_map_t *kwargs){
    ...
    mp_obj_hmac_t *o = m_new_obj_var(mp_obj_hmac_t, digestmod, char, digestmod+sizeof(size_t));
...
static mp_obj_t hmac_HMAC_copy(mp_obj_t self_in) {
    ...
    mp_obj_hmac_t *o = m_new_obj_var(mp_obj_hmac_t, digestmod, char, self->digestmod+sizeof(size_t));
